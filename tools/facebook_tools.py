        ...
        if post_state["facebook"] == False:
            try:
                ...
                if post_response.status_code == 200:
                    post_state["facebook"] = True
                    final_content["facebook"] = message
                    st.success("Facebook post published successfully!")
                    return "Media posted successfully - Facebook!"
                else:
                    return "Failed to post media - Facebook!."
            except Exception as e:
                st.error(f"Failed to post on Facebook: {e}")
                return f"An error occurred: {str(e)}"
        else:
            return "Media posted successfully - Facebook!"