...
def get_tweets_from_llm(content):
        ...
        messages=[
            {
                ...
                "content": dedent("""\
                    ...
                    You extract all the tweets from the text and create a valid JSON array of tweets.
                    Return only the JSON array in the format: ["tweet_1", "tweet_2", ...], where tweet_1 is the
                    first tweet, tweet_2 is the second tweet, and so on. Ensure the JSON is valid and does not
            ...
            },
            {
                "role": "user",
                "content": dedent(f"""\
                    Generate an array of 3 tweets based on the content below.
                    Return only the JSON array of tweets in the format: ["tweet_1", "tweet_2", ...], where tweet_1 is the
                    first tweet, tweet_2 is the second tweet, and so on. Ensure the JSON is valid and does not
                    contain any newline characters or other invalid formatting.
                    Content: {content}
                """),
    ...
    #             "role": "user",
    #             "content": dedent(f"""\
    #                 Generate an array of 1 tweets based on the content below.
